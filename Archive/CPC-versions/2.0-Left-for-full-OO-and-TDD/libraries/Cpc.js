...
class CPC {
    ...
    constructor(dataset, ignoredColumns, svgContainerWidth=600, svgContainerHeight=300, padding=8, panelBackgroundPadding=8, barHeight=80, preferences){
        ...
        this._interBarPadding = 1.0 // takes a value between 0 and 1 // TODO: COULD be added as a parameter
        this._eulerPadding_horizontal = 20 // TODO: COULD be added as a parameter
        this._eulerPadding_vertical = 10 // TODO: COULD be added as a parameter

        // const preferences = []  // TODO: Temporary for debugging; should be removed
        ...
        // Set up initial xScale
        const panel0_xRanges = panel0_ranges.get('x ranges')
        const [ panel0_xRangeStart, panel0_xRangeEnd ] = panel0_xRanges[0]

        const xScaleForInitialPanel = d3.scaleLinear()
                .domain([0, this._currentMaxPossibleValue])
                .rangeRound([ panel0_xRangeStart, panel0_xRangeEnd ])


        // Set up initial yScale
        const panel0_yRanges = panel0_ranges.get('y ranges')
        const [ panel0_yRangeStart, panel0_yRangeEnd ] = panel0_yRanges[0]

        const yScaleForInitialPanel = d3.scaleBand()
            .domain(d3.range(this._datasetSurveyResults.size))
            .rangeRound([panel0_yRangeStart, panel0_yRangeEnd])
            .paddingInner(this._interBarPadding) // takes a value between 0 and 1
            // Old calculation:
            // .rangeRound([0, this._svgContainerHeight-(this._barHeight+this._padding)])

        this._latestFrontYScale = yScaleForInitialPanel
        ...
        if(this._drawContextAsBackground){
            ...
            const start = yScaleForInitialPanel.range()[0]
                , end = yScaleForInitialPanel.range()[1]
            yScaleForInitialPanel.range([start+this._panelBackgroundPadding, end-this._panelBackgroundPadding])
        }
        ...
        this._drawPanel(this._dataset, xScaleForInitialPanel, yScaleForInitialPanel)
        ...
        this._svg.selectAll('.foreground-rectangle')
            .on('click', (d, i, g) => {
                ...
                iterationArray.forEach(i => {
                    ...
                    const updatedFrontRanges = this._calculateRangesForGivenNumberOfPanels(numberOfPanelsThatWillBeVisibleAfterAddition, 'front')  //
                    const updatedFrontXRanges = updatedFrontRanges.get('x ranges')

                    const [ eachUpdatedFrontXRange_start, eachUpdatedFrontXRange_end ] = updatedFrontXRanges[i]

                    const outdatedXScaleFront = this._CpcRegistry.get(eachPanelId)
                          .get('xScaleFront')
                    const updatedXScaleFront = outdatedXScaleFront.rangeRound([eachUpdatedFrontXRange_start, eachUpdatedFrontXRange_end])
                    ...
                    const updatedBackgroundRanges = this._calculateRangesForGivenNumberOfPanels(numberOfPanelsThatWillBeVisibleAfterAddition, 'background')  //
                        , updatedBackgroundXRanges = updatedBackgroundRanges.get('x ranges')

                    const [ eachUpdatedBackgroundRange_start, eachUpdatedBackgroundRange_end ] = updatedBackgroundXRanges[i]
                ...
                const frontScaleRangesForNewPanel = this._calculateRangesForGivenNumberOfPanels(this._deepestPanelDepth + 1, 'front') // +1, because depth count starts from zero
                    , frontXScaleRangesForNewPanel = frontScaleRangesForNewPanel.get('x ranges')
                    , frontYScaleRangesForNewPanel = frontScaleRangesForNewPanel.get('y ranges')

                const [ newPanel_frontXRangeStart, newPanel_frontXRangeEnd ] = frontXScaleRangesForNewPanel[frontXScaleRangesForNewPanel.length-1]  // get last element (newly added range)
                const [ newPanel_frontYRangeStart, newPanel_frontYRangeEnd ] = frontYScaleRangesForNewPanel[frontYScaleRangesForNewPanel.length-1]



                // Get panel background ranges for the current number of visible panels
                const backgroundScaleRangesForNewNumberOfPanels = this._calculateRangesForGivenNumberOfPanels(this._deepestPanelDepth + 1, 'background') // +1, because depth count starts from zero
                    , backgroundXScaleRanges = backgroundScaleRangesForNewNumberOfPanels.get('x ranges')
                    , backgroundYScaleRanges = backgroundScaleRangesForNewNumberOfPanels.get('y ranges')

                const backgroundXScaleRangesForNewPanel = backgroundXScaleRanges[backgroundXScaleRanges.length-1]
                    , backgroundYScaleRangesForNewPanel = backgroundYScaleRanges[backgroundYScaleRanges.length-1]

                const [ newPanel_backgroundXRangeStart, newPanel_backgroundXRangeEnd ] =  backgroundXScaleRangesForNewPanel // get last element (newly added range)
                    , [ newPanel_backgroundYRangeStart, newPanel_backgroundYRangeEnd ] =  backgroundYScaleRangesForNewPanel // get last element (newly added range)


                // Create front xScale for the new panel
                const frontXScaleForNewPanel = d3.scaleLinear()
                    .domain([0, this._currentMaxPossibleValue])
                    .rangeRound([ newPanel_frontXRangeStart, newPanel_frontXRangeEnd ])

                // Create front yScale for the new panel
                const frontYScaleForNewPanel = d3.scaleBand()
                    .domain(d3.range(this._datasetSurveyResults.size))
                    .rangeRound([ newPanel_frontYRangeStart, newPanel_frontYRangeEnd ])
                    .paddingInner(this._interBarPadding)

                this._latestFrontYScale = frontYScaleForNewPanel

                // Create background xScale for the new panel
                const backgroundXScaleForNewPanel = d3.scaleLinear()
                    .domain([0, maximumPossibleColumnValueInPanel])
                    .rangeRound([ newPanel_backgroundXRangeStart, newPanel_backgroundXRangeEnd ])

                // Create background yScale for the new panel
                const backgroundYScaleForNewPanel = d3.scaleBand()
                    .domain(d3.range(2)) // creates [0,1], which corresponds to top and bottom of panel
                    .rangeRound([ newPanel_backgroundYRangeStart, newPanel_backgroundYRangeEnd ])
                    .paddingInner(this._interBarPadding)


                // // Get panel background Y ranges for the current number of visible panels
                // const backgroundYScaleRangesForNewNumberOfPanels = this._calculateYRangesForGivenNumberOfPanels(this._deepestPanelDepth + 1, 'background') // +1, because depth count starts from zero
                // // const [ newPanel_backgroundYRangeStart, newPanel_backgroundYRangeEnd ] = backgroundYScaleRangesForNewNumberOfPanels[backgroundYScaleRangesForNewNumberOfPanels.length-1]  // get last element (newly added range)
                //
                // // Create background yScale for the new panel
                // const backgroundYScaleForNewPanel = d3.scaleBand()
                //     .domain(d3.range(this._datasetSurveyResults.size))
                //     .rangeRound([
                //         0 + this._eulerPadding_vertical,
                //         this._svgContainerHeight - this._eulerPadding_vertical
                //     ])
                //     .paddingInner(1)
                ...
                // Draw the new panel
                this._drawPanel(
                    drilledDownSubsetOfData,
                    frontXScaleForNewPanel,
                    frontYScaleForNewPanel,
                    backgroundXScaleForNewPanel,
                    backgroundYScaleForNewPanel,
                    this._lastClickedCategoryLabel)
     ...
     */
    _drawPanel(data, xScaleFront, yScaleFront, xScaleBackground=null, yScaleBackground=null, backgroundCategoryLabel=null){
        ...
        if(this._drawContextAsBackground){
            ...
            if (!yScaleBackground){
                // Copy the current state of front x scale as the scale for the background
                yScaleBackground = yScaleFront.copy()

                const start = yScaleFront.range()[0]
                    , end = yScaleFront.range()[1]
                yScaleFront.range([start+this._panelBackgroundPadding, end-this._panelBackgroundPadding])
            }
        }



        // Create panel ID and add panel data to registry
        ...
        this._CpcRegistry.set(panelId, new Map())
            ...
            .set('yScaleFront', yScaleFront)
            .set('yScaleBackground', yScaleBackground)
            .set('data', data)
        ...
        surveyResults.forEach((categoriesCountsMap, eachColumnName) => {
            ...
            if (this._drawContextAsBackground || drawingFirstPanelForFirstTimeWithBackground ){
                ...
                const backgroundRectangle = panel.selectAll('.background-rectangle')
                  ...
                  .attr('y', yScaleBackground(0))
                  .attr('height', (d,i) => (yScaleBackground(1) + 300))
                  .attr('fill', d => this._assignColor(d))
                  .attr('opacity', 0.70) // 0.90
                  .attr('z', (d,i,g) => {
                      d3.select(g[i]).lower()
            ...
            const foregroundRectangles = chart.selectAll('.foreground-rectangle')
                ...
                .attr('y', (d,i) => this._latestFrontYScale(chartCount) + this._padding*2 )
                // .attr('y', (d) => yScaleFront(chartCount) + this._padding*2 )
            ...
            const barLabels = chart
                ...
                .attr('y', d => yScaleFront(chartCount) + this._padding*1.5)
    ...
    _updatePanels(){
        ...
        this._CpcRegistry.forEach( (eachValues, eachPanelKey) => {

            const eachPanelRecord = this._CpcRegistry.get(eachPanelKey)
            const eachPanelId = eachPanelRecord.get('id')

            // Get changed variables
            const eachUpdatedXScaleFront = eachPanelRecord.get('xScaleFront')
                , eachUpdatedXScaleBackground = eachPanelRecord.get('xScaleBackground')
                , eachUpdatedYScaleFront = eachPanelRecord.get('yScaleFront')

            //// UPDATE WIDTHS ////

            const allElementsInEachPanel = this._svg
                .select('#' + eachPanelId)

            // Select rectangles in panel
            const frontRectanglesInEachPanel = allElementsInEachPanel
                .selectAll('.foreground-rectangle')


            const chartClassValueDictionary = new Map()
            let chartClasses = []

            // Update foreground rectangles
            frontRectanglesInEachPanel
                .transition()
                .duration(500)
                .attr('x', d => {

                    if (this._drawContextAsBackground){
                        return eachUpdatedXScaleFront(d[0][0])  // TODO: Placeholder; needs to be changed. (First modify the panel resize function so that it can accomodate padding.)
                    }
                    else{
                        return eachUpdatedXScaleFront(d[0][0])
                    }

                })
                .attr('width', d => {

                    return eachUpdatedXScaleFront(d[0][1]) - eachUpdatedXScaleFront(d[0][0])

                })
                .attr('placeholder', (d,i,g) => {

                    const eachChartClass = d3.select(g[i].parentNode).attr('class')
                    if (!chartClasses.includes(eachChartClass)){
                        chartClasses.push(eachChartClass)
                    }
                ...
                .attr('placeholder', (d,i,g) => {
                    ...
                    d3.range(chartClasses.length).forEach( j =>
                        chartClassValueDictionary.set(chartClasses[j], j)
                    )
                })
                .attr('y', (d,i,g) => {

                    const eachChartClass = d3.select(g[i].parentNode).attr('class')
                    const yValueInDomain = chartClassValueDictionary.get(eachChartClass)
                    return this._latestFrontYScale(yValueInDomain)
            ...
            const backgroundRectanglesInEachPanel = allElementsInEachPanel
              .selectAll('.background-rectangle')


            // Update background rectangles
            backgroundRectanglesInEachPanel
                .transition()
                .duration(500)
                .attr('x', d => {

                    if (this._drawContextAsBackground){
                        return eachUpdatedXScaleBackground(d[0][0])  // TODO: Placeholder; needs to be changed. (First modify the panel resize function so that it can accomodate padding.)
                    }
                    else{
                        return eachUpdatedXScaleBackground(d[0][0])
                    }

                })
                .attr('width', d => {

                    return eachUpdatedXScaleBackground(d[0][1]) - eachUpdatedXScaleBackground(d[0][0])
            ...
            // Select labels
            const textsInPanel = allElementsInEachPanel
                .selectAll('text')
            ...
            textsInPanel
                .transition()
                .duration(500)
                .attr('x', d => eachUpdatedXScaleFront(d[0][0]) + 3)
        })
    }



    _calculateRangesForGivenNumberOfPanels (numberOfPanels, eulerMode=null) {
        ...
        const numberOfChartsInPanel = this._datasetSurveyResults.size
        ...
        if (!eulerMode || eulerMode === 'front'){
            ...
            cpcYSpanScale.domain(d3.range(numberOfChartsInPanel))

        }
        else if (eulerMode === 'background'){

            cpcYSpanScale.domain(d3.range(2)) // range(2) creates [0,1], which corresponds to the top and botom of the background rectangle
        ...
        if (!eulerMode){
            ...
            cpcXSpanScale.rangeRound([0, this._svgContainerWidth - horizontalPadding])
            cpcYSpanScale.rangeRound([0 + verticalPadding, this._svgContainerHeight - verticalPadding]) // + 10000
        }
        ...
        d3.range(numberOfPanels).forEach( i => {
            ...
            if (!eulerMode){
                ...
                scaledYRangesForCurrentPanel = [
                    ...
                    cpcYSpanScale(1) - verticalPadding
                ]
